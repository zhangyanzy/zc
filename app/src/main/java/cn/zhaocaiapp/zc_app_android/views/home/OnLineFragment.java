...
public class OnLineFragment extends BaseFragment implements OnRefreshListener, OnLoadmoreListener {
    ...
    @BindView(R.id.home_on_line_refresh)
    RefreshLayout home_on_line_refresh;
    @BindView(R.id.home_on_line_recycler)
    RecyclerView home_on_line_recycler;
    ...
    @Override
    public void init() {
        home_on_line_recycler.setLayoutManager(new LinearLayoutManager(this.getActivity()));

        activityAdapter = new ActivityAdapter(this.getActivity(), activityRespList);
        home_on_line_recycler.setAdapter(activityAdapter);

        home_on_line_refresh.setOnRefreshListener(this);
        home_on_line_refresh.setOnLoadmoreListener(this);
    }
        ...
        HttpUtil.get(Constants.URL.GET_ACTIVITY_LIST, params).subscribe(new BaseResponseObserver<List<ActivityResp>>() {
            ...
            public void success(List<ActivityResp> result) {
                if (pageNumber == 1) {
                    ...
                    //恢复没有更多数据的原始状态
                    home_on_line_refresh.resetNoMoreData();
                } else {
                    activityRespList.addAll(result);
                }
                if (result.size() < Constants.CONFIG.PAGE_SIZE) {
                    //完成加载并标记没有更多数据
                    home_on_line_refresh.finishLoadmoreWithNoMoreData();
                }

                activityAdapter.updata(activityRespList);
                EBLog.i("tag", result.toString());
                home_on_line_refresh.finishRefresh();
            }
